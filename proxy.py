...
bot = telebot.TeleBot('7393436890:AAFMWFhVANLJYHsiN-pWNabBoeFyIIojOTM')
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < COOLDOWN_TIME:
                response = "You Are On Cooldown. Please Wait {time} sec Before Running The /attack Command Again buy premium instant attack with zero sec dm @SAYYED_Z"
                bot.reply_to(message, response)
                return
